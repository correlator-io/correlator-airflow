...
The `airflow-correlator` project uses semantic versioning and automated PyPI releases through GitHub Actions.
...
1. **Create a PR** from a branch with the appropriate prefix:
   ...
   git checkout -b minor/add-task-metadata-extraction

   # Make changes, commit with proper format
   git add .
   git commit -m "minor: Add task metadata extraction"
   git push origin minor/add-task-metadata-extraction
   ```
...
3. Test installation from TestPyPI:
   ```bash
   pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ correlator-airflow
   ```
...
1. **Verify PyPI publication:**
   - Check https://pypi.org/project/correlator-airflow/
...
# Go to: https://github.com/correlator-io/correlator-airflow/releases/new
```
...
**Primary:**
- PyPI: https://pypi.org/project/correlator-airflow/

**Future:**
- Conda-forge (community support)
- Docker Hub (containerized Airflow with plugin)
